...
{
	if (source == JOY3D_AXIS_X || source == JOY3D_AXIS_Y)
		return calc_power(source, read_raw(source));
	else if (source == JOY3D_SRC_BUTTON)
		return read_raw(source) > JOY3D_BTN_ANLOG_THRESHOLD ? 1  : 0;
...
{
	if (source == JOY3D_AXIS_X)
		return x_wire->read();
	else if (source == JOY3D_AXIS_Y)
		return y_wire->read();
	else if (source == JOY3D_SRC_BUTTON)
...
int Joystick::calibrate(int num_levels)
{
	...
	{
		x_val = read_raw(JOY3D_AXIS_X);
		y_val = read_raw(JOY3D_AXIS_Y);
		new_button = read(JOY3D_SRC_BUTTON);
		...
		{
			calibrate_range(JOY3D_AXIS_X, x_val, &x_min, &x_max);
			calibrate_range(JOY3D_AXIS_Y, y_val, &y_min, &y_max);
		}
							   ...
							   int * y_center_min, int * y_center, int * y_center_max)
{
	...
	{
		int sample_x = read_raw(JOY3D_AXIS_X);
		int sample_y = read_raw(JOY3D_AXIS_Y);
