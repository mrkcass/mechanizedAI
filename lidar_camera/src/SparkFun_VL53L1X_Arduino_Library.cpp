...
boolean VL53L1X::begin(i2c_context i2c_bus)
{
   ...
   //Check the device ID
   //mcass - checked in somax code
   uint16_t modelID = readRegister16(VL53L1_IDENTIFICATION__MODEL_ID);
   if (modelID != 0xEACC)
   {
      somax_log_add(SOMAX_LOG_ERR, "VL53L1X: begin. unknown model id: 0x%X", modelID);
      return (false);
   }
   ...
   // result = (result & 0xFE) | 1;
   // printf("VL53L1X: IO voltage mode: 0x%X\n", result);
   ...
   printf("VL53L1X: trim register values:");
...
void VL53L1X::startMeasurement(uint8_t offset)
{
   ...
   while (leftToSend > 0)
   {
      ...
      //i2c_reg16_write_bytes(_i2cPort, address, &configBlock[address - 1 - offset], toSend);

      // for (byte x = 0; x < toSend; x++)
      //    _i2cPort->write(configBlock[address + x - 1 - offset]);
      for (byte x = 0; x < toSend; x++)
         i2c_reg16_write_byte(_i2cPort, address+x, configBlock[address + x - 1 - offset]);
...
{
   uint8_t val = readRegister(VL53L1_GPIO__TIO_HV_STATUS);
   if (val != 0x03)
...
uint8_t VL53L1X::readRegister(uint16_t addr)
{
   ...
   //i2c_dev_write_2bytes(_i2cPort, addr >> 8, addr & 0xFF);
   ...
   return i2c_reg16_read_byte(_i2cPort, addr);
...
uint16_t VL53L1X::readRegister16(uint16_t addr)
{
   ...
   //return (0); //Error: Sensor did not respond
   //uint8_t msb = i2c_dev_read_byte(_i2cPort);
   //uint8_t lsb = i2c_dev_read_byte(_i2cPort);
   //return ((uint16_t)msb << 8 | lsb);
   return i2c_reg16_read_word(_i2cPort, addr);
}
...
boolean VL53L1X::writeRegister(uint16_t addr, uint8_t val)
{
   ...
   //i2c_dev_write_2bytes(_i2cPort, addr >> 8, addr & 0xFF);
   // _i2cPort->write(val);
   //i2c_dev_write_byte(_i2cPort, val);
   // if (_i2cPort->endTransmission() != 0)
   //    return (0); //Error: Sensor did not ACK
   i2c_reg16_write_byte(_i2cPort, addr, val);
   return (1);    //All done!
}

extern int i2c_reg16_write_2bytes(i2c_context i2c, uint16_t address, uint8_t byte0, uint8_t byte1);
...
boolean VL53L1X::writeRegister16(uint16_t addr, uint16_t val)
{
   ...
   // if (_i2cPort->endTransmission() != 0)
   //    return (0); //Error: Sensor did not ACK
   //i2c_dev_write_byte(_i2cPort, val >> 8);
   //i2c_dev_write_byte(_i2cPort, val & 0xFF);
   // uint8_t buff[4];
   // buff[0] = addr >> 8;
   // buff[1] = addr & 0xFF;
   // buff[2] = val >> 8;
   // buff[3] = val & 0xFF;
   // i2c_dev_write_bytes(_i2cPort, buff, 4);
   i2c_reg16_write_2bytes(_i2cPort, addr, val >> 8, val & 0xFF);
   return (1);    //All done!
}