...
static mraa_i2c_context adc_i2c_bus;
...
int adc50_open(int adc50_input)
{
   int return_code = 1;

   adc_i2c_bus = i2c_open(ADC50_I2C_BUS_NUM);

   i2c_set_frequency(adc_i2c_bus, I2C_FREQUENCY_400KHZ);
   ...
   if (configbits != -1)
   {
      ...
      if (adc50_input == ADC50_INPUT_DEFAULT || adc50_input == ADC50_INPUT_JOY3D)
         adc50_init_default();
...
void adc50_close()
{
   if (adc_i2c_bus)
   {
      i2c_close(adc_i2c_bus);
   }
}

int adc50_init_default()
{
   printf("Using: default / jpystick3d configuration\n");
...
int adc50_read_config_register()
{
   i2c_latch_device(adc_i2c_bus, ADC50_I2C_ADDRESS);
   int regbits = i2c_reg_read_word(adc_i2c_bus, ADC50_REG_POINTER_CONFIG);
...
void adc50_write_config_register(int config_bits)
{
   ...
   i2c_reg_write_word(adc_i2c_bus, ADC50_REG_POINTER_CONFIG, reg_bits);
}

int adc50_read_conversion_register()
{
   i2c_latch_device(adc_i2c_bus, ADC50_I2C_ADDRESS);
   int regbits = i2c_reg_read_word(adc_i2c_bus, ADC50_REG_POINTER_CONVERSION);
   return ((regbits & 0xFF) << 8) | ((regbits >> 8) & 0xFF);
}
...
int adc50_testsample()
{
   ...
   while (1)
   {
     ...
     printf("   ADC-A0: %6d   ADC-A1: %6d  ADC-A2: %6d ADC-A3: %6d\r", a0, a1, a2, a3);
     fflush(stdout);
     usleep(1000000/16);
   }
   return 0;
